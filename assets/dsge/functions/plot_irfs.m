function plot_irfs(M_,oo_)
    % Plot IRFs to different shocks
    ...
    var_labels = {'Output growth'; 'Core PCE inflation QoQ'; 'Consumption growth'; 'Federal funds rate'};
    ...
    for jj = 1:size(shocks,1)
        ...
        for ii = 1:size(vars,1)
            ...
            plot((1:Tirf), irf_temp(1:Tirf), 'LineWidth', 2), hold on
            plot((1:Tirf), ss_temp(1:Tirf), 'k--')
            axis tight
            grid minor
            title(var_labels{ii})
            ylabel(ylabel_temp)
